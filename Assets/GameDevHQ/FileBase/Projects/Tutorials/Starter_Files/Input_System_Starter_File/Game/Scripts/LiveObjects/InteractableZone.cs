...
namespace Game.Scripts.LiveObjects
{
    public class InteractableZone : MonoBehaviour
    {
		...
		[SerializeField] private int _keyCodeInput;
		...
		private void OnEnable()
		{
			...
			GameInputManager.OnInteractableKeyE += PressedEKey;
			GameInputManager.OnHoldKey += HoldKey;
			GameInputManager.OnHoldFinished += CancelHold;
		}
		...
		{
			if (_keyCodeInput == 0 && _keyState == KeyState.PressHold && _inHoldState == false)
			{
				_inHoldState = true;
		...
		{
			if (_keyCodeInput == 0 && _keyState == KeyState.PressHold)
			{
				_inHoldState = false;
